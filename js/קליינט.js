...
function nav() {
    document.getElementById("hello").innerText = ` hello ${currentUser.name} `
    ...
    document.getElementById('exit').addEventListener('click', storeSPA.template);
...
function changeColors(id) {
    changeColorToRegularColor();
    document.getElementById(id).style.background = "#3e490a41";
}
function changeColorToRegularColor() {
    document.querySelectorAll(".product").forEach(element => {
        element.style.background = "#A89C38";
    });
}
...
function creatList() {
    ...
    xhrList.f_open('GET', `https://PMTC.co.il/?${currentUser.email}/products/`);
...
function innerHTMLList() {
    ...
    list.forEach(element => {
        listProductPlace.insertAdjacentHTML('beforeend', `<li class="product"id="${element.name}" data-product="${element.name}">${element.name}</li><br>`)
    });
    console.log(list);
    hideProductList();
}
...
const actions = {
    delete: function (e) {
        ...
        if (saveName !== "") {
            ...
            xhrDelete.f_open('DELETE', `https://PMTC.co.il/?${currentUser.email}/product/${saveName}`);
    ...
    add: function (e) {
        ...
        if (product.name !== "" && product.price !== "" && product.QuntityInWerehouse !== "" && product.MinimumAmount !== "" && product.ProductDescription !== "") {
            ...
            xhrAdd.f_open('POST', `https://PMTC.co.il/?${currentUser.email}`);
    ...
    update: function (e) {
        ...
        let saveDetails = document.getElementById('updateQuantity').value;
        if (saveName !== "") {
            const xhrUpdate = new FXMLHttpRequest();
            xhrUpdate.f_open('PUT', `https://PMTC.co.il/?${currentUser.email}/product/${saveName}`);
            xhrUpdate.f_onload = function () {
                if (xhrUpdate.f_status == 200 && xhrUpdate.f_readyState == 4) {
                    console.log();
                    list[myfindIndex(saveName)].QuntityInWerehouse=saveDetails;
                    innerHTMLList();
                    goToHmePage();
                } else {
                    alert("oppssss.... somthig went worng");
    ...
    search: function (e) {
        ...
        if (saveName !== "") {
            ...
            xhrSearch.f_open('GET', `https://PMTC.co.il/?${currentUser.email}/product/${saveName}`);
            xhrSearch.f_send();
